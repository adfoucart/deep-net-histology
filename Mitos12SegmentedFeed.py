...
class MITOS12SegmentedFeed:
	...
	def next_single_batch(self):
		if( self.u == 0 ):
			self.X = np.load(self.batch_files[self.idx[self.current]])
			self.Y = np.load(self.target_files[self.idx[self.current]])
			self.current += 1
			if self.current >= self.n_files : self.current = 0

		self.u += 1
		if( self.u >= 200 ) : self.u = 0

		return [self.X[self.u]], [self.Y[self.u]]