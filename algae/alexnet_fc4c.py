...
class AlexNet:
    def __init__(self, batch_size, im_size, sess, lr, eps, a):
        ...
        # New layers : FullyConnected - FullyConnected - Softmax
        conv4r = tf.reshape(conv4, [batch_size, int(np.prod(conv4.get_shape()[1:]))])

        fc5W = tf.Variable(tf.truncated_normal([int(np.prod(conv4.get_shape()[1:])),5000], stddev=0.1))
        fc5b = tf.Variable(tf.zeros([5000]))
        fc5 = tf.nn.relu6(tf.matmul(conv4r, fc5W) + fc5b)

        fc6W = tf.Variable(tf.truncated_normal([5000,4], stddev=0.1))
        ...
        # Setup Training
        self.train_W = [fc6W, fc5W]
        self.train_b = [fc6b, fc5b]
...
if __name__ == "__main__":
    ...
    with open('%s.log'%clf_date_name, 'wb') as logfile:
        run_info = {'clf_name': clf_name,
                    ...
                    'tile_size': tile_size,
                    'description': "AlexNet, cut at level 4, 1 fully connected relu6 (5000), 1 fully connected relu6 (4) and 1 softmax.",
                    'file': "alexnet_fc4c.py",
                    'lr': lr,
                    'eps': eps,
                    'a': a,
                    'continuing_from_previous_run': True,
                    'restored_clf': "AlexNet_4_1fc_1softmax_201610101417_last.ckpt",
                    'Date': now}
        json.dump(run_info, logfile, indent=4)
        logfile.write("\n")

    saver.restore(sess, "AlexNet_4_1fc_1softmax_201610101417_last.ckpt")
