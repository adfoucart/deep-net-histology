...
## -- Create the Network -- ##
def get_network(X, get_attention=False):
    ...
    flat = tf.reshape(res7, [batch_size.value, int(np.prod(res7.get_shape()[1:]))], name="reshape")

    # Fully connected layers
    fc1 = tf.contrib.layers.fully_connected(flat, 64, activation_fn=tf.nn.relu, weights_initializer=tf.contrib.layers.xavier_initializer(), weights_regularizer=tf.nn.l2_loss, biases_initializer=tf.zeros_initializer(), scope='fc1')
    fc2 = tf.contrib.layers.fully_connected(fc1, 2, activation_fn=tf.nn.softmax, weights_initializer=tf.contrib.layers.xavier_initializer(), weights_regularizer=tf.nn.l2_loss, biases_initializer=tf.zeros_initializer(), scope='fc2')

    if get_attention: return [fc2, res7] # Get the last convolutional layer to see if we can deduce the segmentation from the neuron activation zone
    return fc2
...
def get_sample(data, slide, batch_size, mag, target, balance, tile_size, isTraining):
    ...
    while i < t:
        ...
        if( data[slide][level]['mask'][coord[0]+72:coord[1]-72,coord[2]+72:coord[3]-72].sum() > 0 ):
            ...
            batch_Y[i] = data[slide][level]['mask'][coord[0]+rt[0]:coord[0]+rt[0]+tile_size, coord[2]+rt[1]:coord[2]+rt[1]+tile_size].sum()
            i += 1
...
def train(saver, batch_dir, net, sess, clf_name, loss, merged, train_writer, restore_from, batch_size, hyper_params):
    ...
    Yval = np.load("E:/data/201702-scans/validation/Yval_detect.npy")
    np.random.seed(0)
...
def test_attention(saver, net, res, sess, clf_name, batch_size):
    ts = hyper_params['ts']
    mag = hyper_params['mag']
    tar = hyper_params['tar']
    bal = hyper_params['bal']
    ...
    Xval = np.load("E:/data/201702-scans/validation/Xval_corr.npy")/255
    Yval = np.load("E:/data/201702-scans/validation/Yval_detect_corr.npy")
    Xval_seg = np.load("E:/data/201702-scans/validation/Xval.npy")/255
    Yval_seg = np.load("E:/data/201702-scans/validation/Yval_seg.npy")
    np.random.seed(0)
    #idx = np.arange(Xval.shape[0])
    #np.random.shuffle(idx)
    ...
    for i in range(8):
        Y = res.eval(session=sess, feed_dict={X: Xval_seg[i*batch_size:(i+1)*batch_size]})
        print(Y.shape) # should be [batch_size, h, w, channels]
        Ymax = Y.max(axis=3) # Maximum neuron activation
        Ysum = Y.sum(axis=3) # Sum of all neurons for a particular pixel position

        plt.figure()
        for j in range(4):
            plt.subplot(4,3,j*3+1)
            plt.imshow(Xval_seg[i*batch_size+j])
            plt.subplot(4,3,j*3+2)
            plt.imshow(Yval_seg[i*batch_size+j,:,:])
...
def test_val(saver, net, sess, clf_name, batch_size):
    ...
    Cmat = np.zeros((2,2))
    for i in range(6):
        Y = net.eval(session=sess, feed_dict={X: Xval[i*batch_size:(i+1)*batch_size], target: Yval[i*batch_size:(i+1)*batch_size]})
        pred = Y[:,0]>0.5
        t = Yval[i*batch_size:(i+1)*batch_size,0]>0.5
...
if __name__ == "__main__":
    ...
    if( len(sys.argv) > 1 ):
        action = sys.argv[1]
        batch_size = 21 if action == 'test' else 20
        tile_size = 128
        lr = 3e-5
        eps = 0.1
        a = 0#1e-7
        ...
        if action == 'test_attention':
            net,res = get_network(X, True)
        else:
            net = get_network(X)
        ...
        sess = tf.Session()
        train_writer = tf.summary.FileWriter('e:/data/tf_summaries/slide_defect_detect/%s'%(clf_name), sess.graph)
        ...
        elif action == 'test_attention':
            test_attention(saver, net, res, sess, clf_name, batch_size)
        else:
